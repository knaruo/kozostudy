...
int serial_send_byte(int index, unsigned char c)
{
    ...
    while (!serial_is_send_enable(index));  // wait until send enable
...
unsigned char serial_recv_byte(int index)
{
    ...
    while (!serial_is_recv_enable(index));  // wait until send enable

    c = sci->rdr;
    sci->ssr &= ~H8_3069F_SCI_SSR_RDRF;  // start sending
